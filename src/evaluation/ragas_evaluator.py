        ...
        for model_key in models_to_test:
            ...
            try:
                ...
                if model_key not in MODELS_REGISTRY:
                    print(f"Model '{model_key}' not found in registry. Skipping.")
                    continue
                ...
                self.set_models(llm_model=model_key)
                
                # Create a wrapper function that passes the custom LLM
                ...
                if results_dataset:
                    ...
                    if model_data:
                        ...
                        print(f"Successfully collected results for model: {model_key}")
                else:
                    print(f"Skipping model {model_key} due to evaluation failure.")

            except Exception as e:
                print(f"An error occurred during evaluation for model '{model_key}': {e}")
                if self.debug:
                    import traceback
                    traceback.print_exc()
...
def save_comparison_results(all_results: Dict[str, Any]):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filepath = project_root / "results" / f"ragas_comparison_all_rags_{timestamp}.json"
    ...
    for rag_type in rag_types:
        ...
        for model_key in models_to_test:
            ...
            try:
                ...
                evaluator.set_models(llm_model=model_key)

                # Run evaluation
                results_dataset = evaluator.run_evaluation()

                if results_dataset:
                    model_data = evaluator.save_results(results_dataset, return_data_only=True, model_name=model_key)
                    if model_data:
                        rag_results[model_key] = model_data
                        print(f"✅ Completed: {rag_type} + {model_key}")
                    else:
                        print(f"❌ Failed to save results: {rag_type} + {model_key}")
                else:
                    print(f"❌ Evaluation failed: {rag_type} + {model_key}")

            except Exception as e:
                print(f"❌ Error with {rag_type} + {model_key}: {e}")
                if debug:
                    import traceback
                    traceback.print_exc()
